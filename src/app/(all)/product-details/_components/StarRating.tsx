...
const Star = ({
  ...
  return (
    ...
    >
      ...
      {/* Background */}
      <path d={STAR_PATH} fill="#FFFFFF" stroke="#D6D6D6" strokeWidth="1.2" />

      {/* Yellow fill */}
      <path d={STAR_PATH} fill="#FFE923" clipPath={`url(#${clipId})`} />

      {/* Border */}
      <path d={STAR_PATH} fill="none" stroke="#FFE923" strokeWidth="1.2" />
    </svg>
  );
};
...
type RatingProps = {
  value: number;
  max?: number;
  onChange?: (val: number) => void; // optional interactive handler
};

const Rating = ({ value, max = 5, onChange }: RatingProps) => {
